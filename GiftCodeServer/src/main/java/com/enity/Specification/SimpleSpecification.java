...
public class SimpleSpecification<T> implements Specification<T> {
    ...
    private Predicate generatePredicate(Root<T> root, CriteriaBuilder criteriaBuilder,
            SpecificationOperator op) {
        /*
         * 根据不同的操作符返回特定的查询
         */
        if ("=".equalsIgnoreCase(op.getOper())) {
            ...
            if (op.getValue() instanceof Date) {
                return criteriaBuilder.greaterThanOrEqualTo(root.get(op.getKey()),
                        (Date) op.getValue());
            }
            return criteriaBuilder.ge(root.get(op.getKey()), (Number) op.getValue());
        } else if ("<=".equalsIgnoreCase(op.getOper())) {
            if (op.getValue() instanceof Date) {
                return criteriaBuilder.lessThanOrEqualTo(root.get(op.getKey()),
                        (Date) op.getValue());
            }
